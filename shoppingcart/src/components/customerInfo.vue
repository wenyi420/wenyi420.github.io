  ...
  <div>
    ...
    <div class="form-content">
      <div class="cartlist-content">
        <h3>本次消費總計<span class="price"> NT$ {{getTotalPrice}}</span></h3>
        <p
          class="showCartList"
          @click="showList"
        >查看購買項目</p>
  ...
  export default {
    ...
    methods: {
      async submitCarListInfo() {
        try {
          alert("送出");
          const postAPI = await Vue.axios.post(
          ...
          const customerInfo = await this.$store.commit(
            "SET_CUSTOMER_INFO",
            postAPI.data
          );
          console.log("傳到commit的資料為", customerInfo);
